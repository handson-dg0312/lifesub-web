            ...
            container('sonar-scanner') {
                withSonarQubeEnv('SonarQube') {
                    sh """
                        ...
                        -Dsonar.projectKey=lifesub-web-dg0312 \
                        -Dsonar.projectName=lifesub-web-dg0312 \
                        -Dsonar.sources=src \
                        -Dsonar.tests=src \
        ...
        stage('Generate & Apply Manifest') {
            container('envsubst') {
                sh """
                    ...
                    export lifesub_web_image_path=${props.registry}/${props.image_org}/${props.service_name}:${imageTag}
            ...
            container('azure-cli') {
                sh """
                    ...
                    while [[ -z \$(kubectl -n ${namespace} get svc ${props.service_name} -o jsonpath='{.status.loadBalancer.ingress[0].ip}') ]]; do
                        sleep 5
                    done
                    echo "Service external IP: \$(kubectl -n ${namespace} get svc ${props.service_name} -o jsonpath='{.status.loadBalancer.ingress[0].ip}')"
                """
            }
        }
    }
